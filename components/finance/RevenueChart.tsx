...
export default function RevenueChart({ invoices, expenses, month }: Props) {
  ...
  const data = weeks.map(w => {
    ...
    return { week: w, Revenue: Math.round(revenue), Expenses: Math.round(expense) }
  })

  return (
    <div style={{ backgroundColor: 'var(--bg-surface)', borderRadius: '12px', padding: '20px 24px', border: '1px solid var(--border)' }}>
      <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '16px', margin: '0 0 16px' }}>Revenue vs Expenses</p>
